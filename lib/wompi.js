...
// Convierte un monto USD a COP en centavos (a la tasa fija configurada).
// Wompi requiere "amount-in-cents" como entero.
// Si en el futuro usas COP directamente, multiplicas por 100 nada más.
const USD_TO_COP = parseFloat(process.env.NEXT_PUBLIC_USD_TO_COP_RATE || '4000');
export function toAmountInCents(usdAmount) {
  const cop = Math.round(usdAmount * USD_TO_COP);
  return cop * 100; // centavos
}
...
export function buildCheckoutUrl({
  ...
  const entries = [
    ...
    ['customer-data:phone-number', customerPhoneNumber],
    ...
    ['shipping-address:phone-number', customerPhoneNumber],
  ];
